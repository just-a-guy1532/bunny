...
local angleShit = 0;
local originalAngle = 1.5;
local curAngle = 1.5;

local bagFrameTiming = 0;
local bagSwingFramerate = 12;

function onUpdate(elapsed)
	local animName = getProperty('bag.animation.curAnim.name');
...
function onEvent(tag, value1, value2)
	if tag == 'Knockout Punchbag' then
		...
		onTimerCompleted('kill bag lol', 1, 0)
	end
end
